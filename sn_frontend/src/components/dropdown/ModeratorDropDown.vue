<template>
    <Menu as="div" class="relative inline-block text-left">
      <div>
        <MenuButton class="flex gap-2 items-center dark:bg-slate-700 rounded-lg dark:hover:bg-slate-600 duration-75">
        ...
        >
          <div class="py-1">
            <MenuItem v-slot="{ active }">
              <RouterLink
              :to="{name: 'profile', params: {id: moderator?.information?.id}}"
                :class="[
          ...
          <div class="py-1">
            <MenuItem v-slot="{ active }" @click="$emit('removeModerator')">
              <div
                href="#"
                :class="[
              ...
              >
                <span class="flex items-center gap-3"
                  ><UserMinusIcon class="w-5 h-5" />Hủy tư cách quản trị viên</span
                >
              </div>
            </MenuItem>
          </div>
