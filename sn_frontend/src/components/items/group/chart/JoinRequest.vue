<template>
  <canvas id="joinRequestChart" class="w-full"></canvas>
</template>
...
export default (await import("vue")).defineComponent({
  ...
  methods: {
    async getGroupGrowth() {
      await axios
        .get(`/api/group/${this.$route.params.id}/growth-join-requests/`)
        .then((res) => {
          this.joinRequests = res.data.joinRequests;
          this.setJoinRequestLength(this.joinRequests.length);
          this.createChart();
        })
        .catch((error) => {
          console.log(error);
    ...
    createChart() {
      let joinRequestsInSevenDaysData = [];
      let joinRequestsInTwentyEightDaysData = [];
      let joinRequestsInSixtyDaysData = [];
      ...
      if (this.selectedDays === "7 ngày trước") {
        for (let i = 0; i < timeFrom(7).reverse().length; i++) {
          const length = this.joinRequests.filter(
            (req) =>
              Date.parse(req.created_at) > timeFrom(7).reverse()[i] &&
              Date.parse(req.created_at) < timeFrom(7).reverse()[i + 1]
          ).length;
          const object = {
          ...
          joinRequestsInSevenDaysData.push(object);
        }
      }
      ...
      if (this.selectedDays === "28 ngày trước") {
        for (let i = 27; i >= 0; i -= 7) {
          const length = this.joinRequests.filter(
            (req) =>
              Date.parse(req.created_at) < timeFrom(28).reverse()[i] &&
              Date.parse(req.created_at) > timeFrom(28).reverse()[i - 7]
          ).length;
          const object = {
            x: this.monthAgo[i],
            y: length,
          ...
          joinRequestsInTwentyEightDaysData.push(object);
        }
      }
      ...
      if (this.selectedDays === "60 ngày trước") {
        for (let i = 59; i >= 0; i -= 7) {
          const length = this.joinRequests.filter(
            (req) =>
              Date.parse(req.created_at) < timeFrom(60).reverse()[i] &&
              Date.parse(req.created_at) > timeFrom(60).reverse()[i - 7]
          ).length;
          const object = {
          ...
          joinRequestsInSixtyDaysData.push(object);
        }
      }
      ...
      switch (this.selectedDays) {
        ...
        case "28 ngày trước":
          ...
          break;
        case "60 ngày trước":
          data = joinRequestsInSixtyDaysData.reverse();
