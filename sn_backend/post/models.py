...
class SharePost(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    body = models.TextField(blank=True, null=True)
    post = models.ForeignKey(Post, verbose_name='post_shared', on_delete=models.CASCADE)
    attachments = models.ManyToManyField(PostAttachment, blank=True)
    is_private = models.BooleanField(default=False)
    only_me = models.BooleanField(default=False)
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(User, related_name='share_posts', on_delete=models.CASCADE)
    
    post_to = models.ForeignKey(User, related_name="share_post_to", on_delete=models.CASCADE, null=True)
